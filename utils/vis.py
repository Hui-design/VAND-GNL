...
def tsne_vis_qk(mem, q, gt, name):
    # mem, q需要先进行L2归一化
    mem = F.normalize(mem, p=2, dim=-1)
    q = F.normalize(q, p=2, dim=-1)
    tsne = TSNE(n_components=2) 
    k_tsne = tsne.fit_transform(mem)
    tsne = TSNE(n_components=2)
    q_tsne = tsne.fit_transform(q)
    # np.save(f'memory/{name}', cat_tsne)
    plt.figure() 
    s1 = plt.scatter(k_tsne[:len(mem)//2,0], k_tsne[:len(mem)//2,1], color='r', marker='+', label='normal_k') 
    s2 = plt.scatter(k_tsne[len(mem)//2:,0], k_tsne[:len(mem)//2:,1], color='g', marker='+', label='anomaly_k')   

    s1 = plt.scatter(q_tsne[~gt,0], q_tsne[~gt,1], color='c', marker='o', label='normal_q') 
    s2 = plt.scatter(q_tsne[gt,0], q_tsne[gt,1], color='y', marker='o', label='anomaly_q')   
    plt.legend(loc = 'best')
    plt.savefig(f'memory/{name}_qk.png')
    plt.close('all')


def tsne_vis_cat(mem, name):
    # # mem, q需要先进行L2归一化
    # mem = F.normalize(mem, p=2, dim=-1)
    tsne = TSNE(n_components=2) 
    cat_tsne = tsne.fit_transform(mem)
    np.save(f'memory/{name}', cat_tsne)
    plt.figure() 
    s1 = plt.scatter(cat_tsne[:len(mem)//2,0], cat_tsne[:len(mem)//2,1], color='r') 
    s2 = plt.scatter(cat_tsne[len(mem)//2:,0], cat_tsne[:len(mem)//2:,1], color='g')   
    plt.legend((s1,s2),('normal', 'anomaly') ,loc = 'best')
    plt.savefig(f'memory/{name}_cat.png')
    plt.close('all')

def tsne_vis_multiple(n_mem, a_mem):
    tsne = TSNE(n_components=2) 
    cat_tsne = tsne.fit_transform(np.vstack([n_mem, a_mem]))
    plt.figure() 
    s1 = plt.scatter(cat_tsne[:len(n_mem),0], cat_tsne[:len(n_mem),1], color=(0.75,0.75,1)) 
    s2 = plt.scatter(cat_tsne[len(n_mem):len(n_mem)+len(a_mem),0], cat_tsne[len(n_mem):len(n_mem)+len(a_mem),1], color=(1,0.75,0.75))   
    # normal = {'x': cat_tsne[:len(n_mem),0], 'y': cat_tsne[:len(n_mem),1]}
    # anomaly = {'x': cat_tsne[len(n_mem):len(n_mem)+len(a_mem),0], 'y': cat_tsne[len(n_mem):len(n_mem)+len(a_mem),1]}
    # s1 = sns.scatterplot(data=normal, x='x', y='y', color='blue')
    # s2 = sns.scatterplot(data=anomaly, x='x', y='y', color='red')
    plt.legend((s1,s2),('normal feats', 'anomaly feats') , loc = 'best')
    plt.savefig(f'tsne_cat.png')
    plt.close('all')
